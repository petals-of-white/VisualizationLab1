...
window :: Managed (Maybe Window)
window = managed $ bracket (GLFW.createWindow 800 600 "Hello, Dear ImGui!" Nothing Nothing) (maybe (return ()) GLFW.destroyWindow)
...
setup :: Maybe Window -> IO (Maybe Font)
setup maybeWin = do
  ...
  addFontFromFileTTF_ "C:\\Windows\\Fonts\\arial.ttf" 25

mainLoop :: Window -> Font -> IO ()
